...
# Test 4: Check USB monitoring
echo ""
echo "4. Testing USB monitoring..."
...
echo "   Run GUI:         python3 $SCRIPT_DIR/usb_scanner.py"
echo "   Run headless:    python3 $SCRIPT_DIR/usb_scanner.py --headless"
echo "   Check status:    python3 $SCRIPT_DIR/usb_scanner.py --status"
echo "   Use manager:     ./usb-scanner-manager.sh gui"
EOF
...
main() {
    ...
    # Update requirements.txt for Ubuntu 24.04
    cat > "$SCRIPT_DIR/requirements.txt" << 'EOF'
# Enhanced USB Virus Scanner v2.1 Dependencies
# For Ubuntu 24.04+ - using system packages to avoid externally-managed-environment issues

# System packages (install via apt):
# Core dependencies:
# - python3-pyudev  >= 0.22   (USB device monitoring)
# - python3-tk               (GUI interface)  
# - python3-psutil           (Process management)
...
# sudo apt install python3-pyudev python3-tk python3-psutil clamav clamav-daemon clamav-freshclam dbus-x11 udev psmisc xauth lsof jq

# Note: Ubuntu 24.04+ uses externally-managed Python environments
# Use system packages instead of pip to avoid conflicts
EOF
